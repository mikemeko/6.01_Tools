...
\ref{ch:intro} and various alternatives we considered along the way. First we will
...
problem. By this I mean, given a schematic of a circuit, we start from an empty
protoboard, and search through the space of all possible protoboard layouts to
find the protoboard corresponding to the schematic at hand. Importantly, we
utilize various simplifications and heuristics to facilitate the search as the
space of all possible protoboards is very large.
...
locations. Recall that we want to produce layouts that only contain
...
with this heuristic. In Best First Search, as opposed to in $A*$, we consider
the cost of a vertex to be the value of the heuristic function at that vertex,
and nothing more.

\subsubsection{Limiting the number of expanded vertices}

The $A*$ algorithm works by keeping track of an agenda of vertices to consider
in a priority queue where the value associated with each vertex in the priority
queue is the sum of the cost of the vertex and the value of the heuristic
computed at the state of the vertex. At each step of of the algorithm, we pop
...
\emph{expanding} the vertex. With this
implementation, the algorithm terminates if we either find a solution, or we
exhaust the search space without finding an answer. In our search problem, the
number of states is very large, so this implementation of $A*$ sometimes run out

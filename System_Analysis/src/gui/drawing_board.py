...
class Triangle(Drawable):
  def __init__(self, x1, y1, x2, y2, x3, y3, connectors, fill):
    ...
    self.fill = fill
  def draw_on(self, canvas):
    self.parts.append(canvas.create_polygon(self.x1, self.y1, self.x2, self.y2,
        self.x3, self.y3, fill=self.fill))
...
class Rectangle(Drawable):
  def __init__(self, x1, y1, x2, y2, connectors, fill):
    ...
    self.fill = fill
  def draw_on(self, canvas):
    self.parts.append(canvas.create_rectangle(self.bounding_box,
        fill=self.fill))
